...
class MultiScaleSSIMLoss(_Loss):
    ...
    def forward(self, input, target, weights=[0.0448, 0.2856, 0.3001, 0.2363, 0.1333], max_val=1.):
        return 1-ms_ssim_loss(input, target, max_val=max_val, k1=self.k1, k2=self.k2, sigma=self.sigma, kernel=self.kernel,
...
class MultiScaleSSIMLoss3d(_Loss):
    ...
    def forward(self, input, target, weights=[0.0448, 0.2856, 0.3001, 0.2363, 0.1333], max_val=1.):
        return 1-ms_ssim_loss3d(input, target, max_val=max_val, k1=self.k1, k2=self.k2, sigma=self.sigma, kernel=self.kernel,
                              weights=weights, filter_size=self.filter_size, reduction=self.reduction)
